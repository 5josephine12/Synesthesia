...
test("keeps visual effects bounded and free of production diagnostics", async () => {
  ...
  assert.match(styleThreeSource, /context\.stroke\(shape\.edge\)/);
  assert.match(styleThreeSource, /rgba\(255, 255, 255/);
  assert.match(styleThreeSource, /rgba\(4, 5, 8/);
  assert.match(styleThreeSource, /const MAX_VISIBLE_GROWTHS = 8/);
  assert.match(styleThreeSource, /const RENDER_PIXEL_BUDGET = 1_050_000/);
  assert.match(styleThreeSource, /desynchronized: true/);
  assert.match(styleThreeSource, /Math\.pow\(Math\.max\(0\.003, 1 - normalized\), 0\.43\)/);
  assert.match(styleThreeSource, /context\.globalCompositeOperation = "destination-out"/);
  assert.match(styleThreeSource, /const details: Path2D\[\] = \[\]/);
