...
test("keeps visual effects bounded and free of production diagnostics", async () => {
  ...
  assert.match(styleThreeSource, /function smoothClosedPath/);
  assert.match(styleThreeSource, /function buildBranch/);
  assert.match(styleThreeSource, /const branchCount = 6 \+ Math\.floor/);
  assert.match(styleThreeSource, /context\.fill\(branch\.outline\)/);
  assert.match(styleThreeSource, /context\.stroke\(branch\.outline\)/);
  assert.match(styleThreeSource, /rgba\(255, 255, 255/);
  assert.match(styleThreeSource, /rgba\(8, 9, 12/);
  assert.match(styleThreeSource, /const MAX_VISIBLE_ORGANISMS = 8/);
  assert.match(styleThreeSource, /const RENDER_PIXEL_BUDGET = 1_050_000/);
  assert.match(styleThreeSource, /desynchronized: true/);
  assert.match(styleThreeSource, /Math\.pow\(Math\.max\(0\.018, 1 - normalized\), 0\.48\)/);
  assert.match(styleThreeSource, /const innerLines: Path2D\[\] = \[\]/);
