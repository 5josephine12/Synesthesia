...
const telemetryRendererReady = import("./art-styles/telemetry").then((renderer) => {
  telemetryRenderer = renderer;
  return renderer;
});
...
type LiquidMetalRenderer = typeof import("./art-styles/style-4");
let liquidMetalRenderer: LiquidMetalRenderer | null = null;
let liquidMetalRendererReady: Promise<LiquidMetalRenderer> | null = null;
function loadLiquidMetalRenderer() {
  liquidMetalRendererReady ??= import("./art-styles/style-4").then((renderer) => {
    liquidMetalRenderer = renderer;
    return renderer;
  });
  return liquidMetalRendererReady;
}
...
const METALHEART_FORMATION_DURATION = 620;
...
function drawAuraParticle(
  ...
  if (artStyle === "style-3") {
    const age = formationAge ?? Math.max(0, now - blob.createdAt);
    metalheartRenderer?.drawMetalheartParticle(context, {
      seed: blob.id * 4099 + blob.midi * 131 + blob.repeat * 17,
      midi: blob.midi,
      radius,
      alpha,
      arrival: easeOutCubic(age / METALHEART_FORMATION_DURATION),
      time: Math.min(age, METALHEART_FORMATION_DURATION),
      stretch: blob.stretch,
      thickness: blob.thickness,
...
function drawAuraComposition(
  ...
  for (let index = startIndex; index < limit; index += 1) {
    ...
    const formationDuration = artStyle === "style-3"
      ? METALHEART_FORMATION_DURATION
      : artStyle === "style-4"
    ...
    context.globalCompositeOperation =
      artStyle === "style-2" || artStyle === "style-3" || artStyle === "style-4"
        ? "source-over"
        : blob.blendMode;
    drawAuraParticle(
      context,
...
function drawChronologicalAuraLayers(
  ...
  while (runStart < visibleLimit) {
    const runStyle = blobs[runStart].artStyle ?? fallbackArtStyle;
    const runKind = runStyle === "style-2"
      ? "pixel"
      : runStyle === "style-3"
        ? "metalheart"
        : runStyle === "style-4"
    ...
    while (runEnd < visibleLimit) {
      ...
      const nextKind = nextStyle === "style-2"
        ? "pixel"
        : nextStyle === "style-3"
          ? "metalheart"
          : nextStyle === "style-4"
    ...
    if (runKind === "pixel") {
      ...
      // A restrained screen glow supplies the luminous reflection while the
      // source-over pass retains the sharp, machined plate edges.
      blurredContext.clearRect(0, 0, blurredLayer.width, blurredLayer.height);
      blurredContext.save();
      blurredContext.filter = `blur(${Math.max(0.5, blurRadius * 0.42)}px)`;
...
export function AuraToy() {
  ...
  const metalheartPulseRef = useRef<MetalheartPulse>({
  ...
  const spawnBlob = useCallback((
    ...
    const liquidMetalAnchor = LIQUID_METAL_COMPOSITION_ANCHORS[
      modulo(
        compositionIndex + note.pc * 2 + modeIndex,
        LIQUID_METAL_COMPOSITION_ANCHORS.length,
      )
    ];
    const baseAngle =
    ...
    const x = currentArtStyle === "style-2"
      ...
      : currentArtStyle === "style-3"
        ...
        : currentArtStyle === "style-4"
          ? clamp(
              lerp(baseX, liquidMetalAnchor[0], 0.82) +
                (identityRng() - 0.5) * 0.026,
              0.12,
              0.88,
            )
          : clamp(
              baseX +
                Math.cos(trailDirection) * trailDistance +
                Math.cos(trailDirection + Math.PI / 2) * trailBend,
              0.035,
              0.965,
            );
    const y = currentArtStyle === "style-2"
      ...
      : currentArtStyle === "style-3"
        ...
        : currentArtStyle === "style-4"
          ? clamp(
              lerp(baseY, liquidMetalAnchor[1], 0.82) +
                (identityRng() - 0.5) * 0.024,
              0.12,
              0.82,
            )
          : clamp(
    ...
    const stretch =
      currentArtStyle === "style-2"
        ...
        : currentArtStyle === "style-3"
          ? lerp(1.65, 3.15, identityRng()) * (0.94 + Math.min(repeat, 6) * 0.025)
    ...
    const nextBlob: BlobParticle = {
      ...
      angle:
        currentArtStyle === "style-2"
          ...
          : currentArtStyle === "style-3"
            ? baseAngle * 0.72 + (identityRng() - 0.5) * 0.32
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      while (settledCount < blobsRef.current.length) {
        ...
        const formationDuration = blobStyle === "style-3"
          ? METALHEART_FORMATION_DURATION
          : blobStyle === "style-4"
            ? LIQUID_METAL_FORMATION_DURATION
            : BLOB_ARRIVAL_DURATION;
        if (!reducedMotionRef.current && now - blob.createdAt < formationDuration) break;
        if (blobStyle !== "style-2" && blobStyle !== "style-3" && blobStyle !== "style-4") {
          drawAuraComposition(
            settledContext,
            blobsRef.current,
            settled.width,
            settled.height,
            blob.createdAt + formationDuration,
            undefined,
            settledCount,
            settledCount + 1,
            currentArtStyle,
      ...
      const containsOrganicStyle = hasAuraStyle || hasMetalheartStyle || hasLiquidMetalStyle;
      ...
      const blurRadius =
        (containsOrganicStyle || hasCompactedHistory
          ...
          : reducedMotionRef.current
            ? 0.4
            : 0.65) * blurScale;
      if (!hasDottedStyle && !hasMetalheartStyle && !hasLiquidMetalStyle) {
      ...
      if (
        ...
        hasMetalheartStyle ||
        hasLiquidMetalStyle ||
        (hasDottedStyle && containsOrganicStyle)
      ) {
        drawChronologicalAuraLayers(
          context,
          offscreen,
      ...
      const nextTelemetryFrameAt = telemetryOpacity > 0.001
        ? (telemetryRenderer?.telemetryNextFrameAt(now) ?? Number.POSITIVE_INFINITY)
        : Number.POSITIVE_INFINITY;

      const newestBlob = blobsRef.current.at(-1);
      const newestFormationDuration =
        (newestBlob?.artStyle ?? currentArtStyle) === "style-3"
          ? METALHEART_FORMATION_DURATION
          : (newestBlob?.artStyle ?? currentArtStyle) === "style-4"
            ? LIQUID_METAL_FORMATION_DURATION
            : BLOB_ARRIVAL_DURATION;
    ...
    return () => {
      ...
      liquidMetalRenderer?.disposeLiquidMetalRenderer();
    };
  }, []);
  ...
  const selectArtStyleByIndex = useCallback(
    (nextIndex: number) => {
      ...
      } else if (nextStyle.id === "style-4") {
        void loadLiquidMetalRenderer().then((renderer) => {
          renderer.warmLiquidMetalRenderer();
          wakeRendererRef.current?.();
        });
      }
      wakeRendererRef.current?.();
